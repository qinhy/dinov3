...
class ConvModule(nn.Module):
    ...
    Args:
        ...
        kernel_size (Union[int, tuple[int]]): Size of the convolving kernel.
            Same as that in ``nn._ConvNd``.
        stride (Union[int, tuple[int]]): Stride of the convolution.
            Same as that in ``nn._ConvNd``.
        padding (Union[int, tuple[int]]): Zero-padding added to both sides of
            the input. Same as that in ``nn._ConvNd``.
        dilation (Union[int, tuple[int]]): Spacing between kernel elements.
            Same as that in ``nn._ConvNd``.
